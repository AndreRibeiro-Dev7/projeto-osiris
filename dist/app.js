...
function openBarberAccessModal(barberId) { const barber = state.barbers.find((item) => item.id === barberId); if (!barber) return; const account = state.barberAccounts.find((item) => item.barber_id === barberId); state.accessBarberId = barberId; $("barber-access-form").reset(); $("barber-access-title").textContent = account ? "Gerenciar acesso" : "Criar acesso"; $("barber-access-professional").textContent = barber.full_name; $("barber-access-email").value = account?.email || ""; $("barber-access-password").required = !account; $("barber-access-password-label").textContent = account ? "Nova senha (opcional)" : "Senha inicial"; $("barber-access-password").placeholder = account ? "Deixe vazio para manter a senha" : "Mínimo de 12 caracteres"; $("barber-access-status-field").hidden = !account; $("barber-access-active").checked = account?.is_active ?? true; $("delete-barber-access").hidden = !account; $("save-barber-access-label").textContent = account ? "Salvar acesso" : "Criar acesso"; $("barber-access-error").textContent = ""; $("barber-access-modal").hidden = false; document.body.classList.add("modal-open"); setTimeout(() => $("barber-access-email").focus(), 0); }
