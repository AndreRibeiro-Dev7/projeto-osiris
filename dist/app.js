...
function renderAppointments(items) {
  ...
  const visible = items.filter((appointment) => { const customer = state.customers.find((item) => item.id === appointment.customer_id); const barber = state.barbers.find((item) => item.id === appointment.barber_id); const matchesStatus = status === "all" || appointment.status === status; const matchesQuery = !query || customer?.full_name.toLocaleLowerCase("pt-BR").includes(query) || barber?.full_name.toLocaleLowerCase("pt-BR").includes(query) || (queryDigits && customer?.phone.replace(/\D/g, "").includes(queryDigits)); return matchesStatus && matchesQuery; });
  ...
  $("appointments").innerHTML = visible.map((a) => { const customer = state.customers.find((c) => c.id === a.customer_id); const service = state.services.find((s) => s.id === a.service_id); const barber = state.barbers.find((b) => b.id === a.barber_id); const active = ["scheduled", "confirmed"].includes(a.status); return `<article class="appointment"><div class="time">${formatTime(a.starts_at)}</div><div class="person"><strong>${customer?.full_name || "Cliente"}</strong><span>${service?.name || a.notes || "Atendimento"} · ${barber?.full_name || "Profissional"} · até ${formatTime(a.ends_at)}${a.price_cents != null ? ` · ${formatMoney(a.price_cents)}` : ""}</span></div><span class="badge ${a.status}">${statusLabel(a.status)}</span><div class="actions"><button data-detail-id="${a.id}">Detalhes</button>${a.status === "scheduled" ? `<button data-action="confirm" data-id="${a.id}">Confirmar</button>` : ""}${active ? `<button data-reschedule-id="${a.id}">Reagendar</button><button data-reminder-id="${a.id}">WhatsApp</button><button data-complete-id="${a.id}">Concluir</button><button data-action="no-show" data-id="${a.id}">Não veio</button><button data-action="cancel" data-id="${a.id}">Cancelar</button>` : ""}</div></article>`; }).join("");
}
...
function openAppointmentDetail(id) {
  ...
  $("appointment-detail-title").textContent = customer?.full_name || "Cliente"; $("appointment-detail-subtitle").textContent = `${formatAppointmentDate(appointment.starts_at)} · ${formatTime(appointment.starts_at)} às ${formatTime(appointment.ends_at)}`;
  $("appointment-detail-content").innerHTML = `<dl><div><dt>Telefone</dt><dd>${escapeHtml(customer?.phone || "Não informado")}</dd></div><div><dt>Profissional</dt><dd>${escapeHtml(barber?.full_name || "Não informado")}</dd></div><div><dt>Serviço</dt><dd>${escapeHtml(service?.name || "Atendimento")}</dd></div><div><dt>Status</dt><dd><span class="badge ${appointment.status}">${statusLabel(appointment.status)}</span></dd></div><div><dt>Valor</dt><dd>${appointment.price_cents != null ? formatMoney(appointment.price_cents) : "Não informado"}</dd></div></dl>`;
  $("appointment-detail-notes").value = appointment.notes || ""; $("appointment-detail-error").textContent = "";
  $("appointment-detail-whatsapp").hidden = !customer?.phone; $("appointment-detail-whatsapp").dataset.phone = customer?.phone || ""; $("appointment-detail-modal").hidden = false; document.body.classList.add("modal-open");
}
...
function renderBirthdays() {
  ...
  $("birthday-list").innerHTML = birthdays.length ? birthdays.map(({ customer, days, date }) => { const when = days === 0 ? "Hoje" : days === 1 ? "Amanhã" : `Em ${days} dias`; const formatted = new Intl.DateTimeFormat("pt-BR", { day: "2-digit", month: "short", timeZone: "UTC" }).format(date); return `<article class="birthday-card"><div class="birthday-date"><strong>${date.getUTCDate()}</strong><span>${formatted.split(" de ").pop()}</span></div><div><h3>${customer.full_name}</h3><p>${when}</p></div><button class="card-action" data-birthday-whatsapp="${customer.id}" type="button">Parabenizar</button></article>`; }).join("") : '<div class="empty birthday-empty">Nenhum aniversário nos próximos 30 dias.</div>';
}
...
function renderCustomerRanking() {
  ...
  $("customer-ranking-list").innerHTML = ranking.length ? ranking.map((item, index) => `<article class="ranking-card"><span class="ranking-position">${index + 1}º</span><div class="avatar">${initials(item.full_name)}</div><div><h3>${item.full_name}</h3><p>${item.completed} visita${item.completed === 1 ? "" : "s"} concluída${item.completed === 1 ? "" : "s"}</p></div><strong>${formatMoney(item.total_spent_cents)}</strong><button class="card-action" data-ranking-history-id="${item.id}" type="button">Ver histórico</button></article>`).join("") : '<div class="empty ranking-empty">O ranking aparecerá após o primeiro atendimento concluído.</div>';
}
...
async function openCustomerHistory(customerId) {
  ...
  $("history-loyalty").innerHTML = loyalty?.loyalty_enabled ? `<section class="loyalty-card"><div><span>FIDELIDADE</span><strong>${loyalty.loyalty_progress} de ${loyalty.loyalty_target} pontos</strong><small>Recompensa: ${loyalty.loyalty_reward} · saldo ${loyalty.loyalty_rewards_available}</small></div><div class="loyalty-track"><i style="width:${loyaltyWidth}%"></i></div>${loyalty.loyalty_rewards_available > 0 ? `<button class="primary loyalty-redeem" data-redeem-loyalty="${customerId}" type="button"><span>Resgatar recompensa</span><span>→</span></button>` : ""}</section>` : "";
...
function openAppointmentModal() {
  ...
  $("appointment-customer").innerHTML = state.customers.map((c) => `<option value="${c.id}">${c.full_name}</option>`).join("");
  $("appointment-barber").innerHTML = state.barbers.filter((b) => b.is_active).map((b) => `<option value="${b.id}" ${b.id === $("barber-filter").value ? "selected" : ""}>${b.full_name}</option>`).join("");
  $("appointment-service").innerHTML = state.services.filter((s) => s.is_active).map((s) => `<option value="${s.id}">${s.name} · ${s.duration_minutes} min · ${formatMoney(s.price_cents)}</option>`).join("");
...
function openRescheduleModal(id) {
  ...
  $("reschedule-barber").innerHTML = state.barbers.filter((item) => item.is_active).map((item) => `<option value="${item.id}" ${item.id === appointment.barber_id ? "selected" : ""}>${item.full_name}</option>`).join("");
  $("reschedule-service").innerHTML = state.services.filter((item) => item.is_active).map((item) => `<option value="${item.id}" ${item.id === appointment.service_id ? "selected" : ""}>${item.name} · ${item.duration_minutes} min</option>`).join("");
...
function sendWhatsAppReminder(id) {
  ...
  if (!customer?.phone) { showToast("Este cliente não possui WhatsApp cadastrado."); return; }
  ...
  const greetingName = customer.full_name.split(" ")[0]; const confirmationLink = `${location.origin}/dashboard/booking/?booking_token=${appointment.public_token}`; const message = `Olá, ${greetingName}! Lembramos que você tem ${service?.name || "um atendimento"} agendado na ${state.business.name}, ${formatAppointmentDate(appointment.starts_at)}, às ${formatTime(appointment.starts_at)}. Confirme sua presença ou cancele pelo link: ${confirmationLink}`;
  window.open(`https://wa.me/${phone}?text=${encodeURIComponent(message)}`, "_blank", "noopener");
}
function logout() { sessionStorage.removeItem("osiris_token"); state.token = null; $("loading-view").hidden = true; $("app-view").hidden = true; $("login-view").hidden = false; window.scrollTo(0, 0); }
...
$("team").addEventListener("click", (event) => { const editButton = event.target.closest("[data-edit-barber-id]"); if (editButton) { openBarberEditModal(editButton.dataset.editBarberId); return; } const toggleButton = event.target.closest("[data-toggle-barber-id]"); if (toggleButton) { toggleBarber(toggleButton.dataset.toggleBarberId, toggleButton.dataset.active === "true"); return; } const timeOffButton = event.target.closest("[data-time-off-id]"); if (timeOffButton) { openTimeOffModal(timeOffButton.dataset.timeOffId); return; } const scheduleButton = event.target.closest("[data-schedule-id]"); if (scheduleButton) openScheduleModal(scheduleButton.dataset.scheduleId); }); $("close-schedule").addEventListener("click", closeScheduleModal); $("cancel-schedule").addEventListener("click", closeScheduleModal); $("schedule-form").addEventListener("submit", saveSchedule); $("schedule-modal").addEventListener("click", (event) => { if (event.target === $("schedule-modal")) closeScheduleModal(); });
