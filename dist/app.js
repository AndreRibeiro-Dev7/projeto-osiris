...
async function boot() {
  ...
  try {
    ...
    [state.business, state.barbers, state.barberAccounts, state.customers, state.customerPortfolio, state.services] = await Promise.all([
      request(`/businesses/${state.me.business_id}`), request(`/businesses/${state.me.business_id}/barbers`), request("/auth/barber-accounts"), request(`/businesses/${state.me.business_id}/customers`), request(`/businesses/${state.me.business_id}/customers/portfolio`), request(`/businesses/${state.me.business_id}/services`)
    ]);
    document.body.classList.remove("barber-session"); document.querySelectorAll(".nav-item").forEach((button) => { button.hidden = false; button.classList.toggle("active", button.dataset.view === "inicio"); }); document.querySelectorAll(".panel-view").forEach((panel) => { panel.hidden = panel.id !== "inicio-panel"; }); $("open-account-security").hidden = false; $("barber-filter").disabled = false; $("new-appointment").hidden = false; $("save-appointment-notes").hidden = false; document.querySelector("#agenda-panel .title-row h1").textContent = "Agenda"; document.querySelector("#agenda-panel .title-row .muted").textContent = "Organize os atendimentos por profissional e data.";
...
async function bootBarber() {
  ...
  $("open-account-security").hidden = true;
...
function renderPeople() {
  ...
  $("team").innerHTML = state.barbers.map((p) => { const account = state.barberAccounts.find((item) => item.barber_id === p.id); const access = account ? `<button class="card-action access-created ${account.is_active ? "" : "suspended"}" data-barber-manage-id="${p.id}" type="button" title="${escapeHtml(account.email)}">${account.is_active ? "✓ Acesso já criado" : "⊘ Acesso desativado"}</button>` : `<button class="card-action" data-barber-access-id="${p.id}" type="button">Criar acesso</button>`; return `<article class="person-card ${p.is_active ? "" : "inactive"}"><div class="avatar">${escapeHtml(initials(p.full_name))}</div><h3>${escapeHtml(p.full_name)}</h3><p>${escapeHtml(p.phone)}</p><small>${p.is_active ? "● Ativo" : "Inativo"} · Comissão ${p.commission_percentage || 0}%</small><div class="card-actions"><button class="card-action" data-edit-barber-id="${p.id}" type="button">Editar profissional</button><button class="card-action" data-toggle-barber-id="${p.id}" data-active="${p.is_active}" type="button">${p.is_active ? "Desativar" : "Ativar"}</button><button class="card-action" data-time-off-id="${p.id}" type="button">Bloquear agenda</button>${access}<button class="card-action wide" data-schedule-id="${p.id}" type="button">Horários e comissão</button></div></article>`; }).join("") || '<div class="empty">Nenhum profissional cadastrado.</div>';
...
function openBarberAccessModal(barberId) { const barber = state.barbers.find((item) => item.id === barberId); if (!barber) return; const account = state.barberAccounts.find((item) => item.barber_id === barberId); state.accessBarberId = barberId; $("barber-access-form").reset(); $("barber-access-title").textContent = account ? "Gerenciar acesso" : "Criar acesso"; $("barber-access-professional").textContent = barber.full_name; $("barber-access-email").value = account?.email || ""; $("barber-owner-password-field").hidden = !account; $("barber-owner-password").required = Boolean(account); $("barber-access-password").required = !account; $("barber-access-password-label").textContent = account ? "Nova senha (opcional)" : "Senha inicial"; $("barber-access-password").placeholder = account ? "Deixe vazio para manter a senha" : "Mínimo de 12 caracteres"; $("barber-access-password-confirm").required = !account; $("barber-access-password-confirm-field").querySelector("span").textContent = account ? "Digite novamente a nova senha" : "Digite novamente a senha inicial"; $("barber-access-status-field").hidden = !account; $("barber-access-active").checked = account?.is_active ?? true; $("delete-barber-access").hidden = !account; $("save-barber-access-label").textContent = account ? "Salvar acesso" : "Criar acesso"; $("barber-access-error").textContent = ""; $("barber-access-modal").hidden = false; document.body.classList.add("modal-open"); setTimeout(() => $("barber-access-email").focus(), 0); }
...
async function createBarberAccess(event) { event.preventDefault(); const existing = state.barberAccounts.find((item) => item.barber_id === state.accessBarberId); const password = $("barber-access-password").value; const confirmation = $("barber-access-password-confirm").value; $("barber-access-error").textContent = ""; if (password !== confirmation) { $("barber-access-error").textContent = "As duas senhas não são iguais. Confira e tente novamente."; return; } $("save-barber-access").disabled = true; $("save-barber-access-label").textContent = existing ? "Salvando..." : "Criando..."; try { const payload = existing ? { current_password: $("barber-owner-password").value, email: $("barber-access-email").value.trim(), is_active: $("barber-access-active").checked, password: password || null, password_confirmation: confirmation || null } : { barber_id: state.accessBarberId, email: $("barber-access-email").value.trim(), password }; const account = await request(existing ? `/auth/barber-accounts/${state.accessBarberId}` : "/auth/barber-accounts", { method: existing ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }); if (existing) state.barberAccounts = state.barberAccounts.map((item) => item.id === account.id ? account : item); else state.barberAccounts.push(account); closeBarberAccessModal(); renderPeople(); showToast(existing ? "Acesso atualizado com sucesso." : "Acesso do profissional criado com sucesso."); } catch (error) { $("barber-access-error").textContent = error.message === "Current owner password is incorrect." ? "A senha atual do proprietário está incorreta." : error.message === "This professional already has a login account." ? "Este profissional já possui um acesso." : error.message === "An account with this email already exists." ? "Este e-mail já está sendo usado." : error.message; } finally { $("save-barber-access").disabled = false; $("save-barber-access-label").textContent = existing ? "Salvar acesso" : "Criar acesso"; } }
async function deleteBarberAccess() { const barber = state.barbers.find((item) => item.id === state.accessBarberId); const barberId = state.accessBarberId; const confirmed = await confirmAction({ eyebrow: "ACESSO DO PROFISSIONAL", title: "Excluir acesso", detail: barber?.full_name || "Profissional", message: "O histórico e os agendamentos serão preservados, mas este profissional não poderá mais entrar no sistema.", confirmLabel: "Excluir acesso", variant: "danger" }); if (!confirmed) { document.body.classList.add("modal-open"); return; } try { await request(`/auth/barber-accounts/${barberId}`, { method: "DELETE" }); state.barberAccounts = state.barberAccounts.filter((item) => item.barber_id !== barberId); closeBarberAccessModal(); renderPeople(); showToast("Acesso excluído. O histórico foi preservado."); } catch (error) { document.body.classList.add("modal-open"); $("barber-access-error").textContent = error.message; } }
...
function openAccountSecurity() { $("account-email-form").reset(); $("account-password-form").reset(); $("account-security-email").textContent = `Conta atual: ${state.me.email}`; $("account-email-error").textContent = ""; $("account-password-error").textContent = ""; $("account-security-modal").hidden = false; document.body.classList.add("modal-open"); }
function closeAccountSecurity() { $("account-security-modal").hidden = true; document.body.classList.remove("modal-open"); }
async function changeAccountEmail(event) { event.preventDefault(); const newEmail = $("account-new-email").value.trim().toLowerCase(); $("account-email-error").textContent = ""; if (newEmail === state.me.email.toLowerCase()) { $("account-email-error").textContent = "Informe um e-mail diferente do atual."; return; } $("save-account-email").disabled = true; $("save-account-email-label").textContent = "Verificando..."; try { const result = await request("/auth/email/request", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ current_password: $("account-email-current-password").value, new_email: newEmail }) }); state.pendingEmail = newEmail; $("email-code-destination").textContent = `Enviado para ${newEmail}`; $("email-code-hint").textContent = result.development_code ? `Modo de desenvolvimento — código: ${result.development_code}` : "O código expira em 10 minutos."; $("email-code").value = ""; $("email-confirm-error").textContent = ""; $("email-confirm-modal").hidden = false; document.body.classList.add("modal-open"); setTimeout(() => $("email-code").focus(), 0); } catch (error) { $("account-email-error").textContent = error.message.includes("Current password") ? "A senha atual está incorreta." : error.message.includes("already exists") ? "Este e-mail já está sendo utilizado." : error.message.includes("could not be sent") ? "Não foi possível enviar o e-mail de confirmação." : error.message; } finally { $("save-account-email").disabled = false; $("save-account-email-label").textContent = "Alterar e-mail"; } }
async function changeAccountPassword(event) { event.preventDefault(); const currentPassword = $("account-current-password").value; const newPassword = $("account-new-password").value; const confirmation = $("account-confirm-password").value; $("account-password-error").textContent = ""; if (newPassword !== confirmation) { $("account-password-error").textContent = "As duas novas senhas não são iguais. Confira e tente novamente."; return; } if (currentPassword === newPassword) { $("account-password-error").textContent = "A nova senha deve ser diferente da senha atual."; return; } $("save-account-password").disabled = true; $("save-account-password-label").textContent = "Alterando..."; try { await request("/auth/password", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ current_password: currentPassword, new_password: newPassword }) }); $("account-password-form").reset(); showToast("Senha alterada. Entre novamente com a nova senha."); setTimeout(logout, 1500); } catch (error) { $("account-password-error").textContent = error.message.includes("Current password") ? "A senha atual está incorreta." : error.message; } finally { $("save-account-password").disabled = false; $("save-account-password-label").textContent = "Alterar senha"; } }

function closeEmailConfirmation() { $("email-confirm-modal").hidden = true; document.body.classList.toggle("modal-open", !$("account-security-modal").hidden); }
...
async function confirmOwnerEmail(event) {
  ...
  try {
    ...
    state.me = updated; closeEmailConfirmation(); closeAccountSecurity(); $("email-form").reset(); $("account-email-form").reset(); $("current-email").value = updated.email; showToast("E-mail confirmado. Entre novamente com o novo endereço."); setTimeout(logout, 1500);
...
$("login-form").addEventListener("submit", login); $("logout").addEventListener("click", logout); $("logout-header").addEventListener("click", logout); $("open-account-security").addEventListener("click", openAccountSecurity); $("close-account-security").addEventListener("click", closeAccountSecurity); $("account-security-modal").addEventListener("click", (event) => { if (event.target === $("account-security-modal")) closeAccountSecurity(); }); $("account-email-form").addEventListener("submit", changeAccountEmail); $("account-password-form").addEventListener("submit", changeAccountPassword); $("refresh").addEventListener("click", loadAgenda); $("date-filter").addEventListener("change", loadAgenda); $("barber-filter").addEventListener("change", loadAgenda);
