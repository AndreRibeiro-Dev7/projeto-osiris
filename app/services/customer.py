...
CUSTOMER_PHONE_ALREADY_EXISTS = (
    ...
    "Localize o cliente existente ou informe outro número."
)
...
class CustomerService:
    ...
    async def create(self, business_id: UUID, payload: CustomerCreate) -> Customer:
        ...
        if await self._customers.get_by_business_and_phone(business_id, payload.phone):
            raise DuplicateResourceError(CUSTOMER_PHONE_ALREADY_EXISTS)
        ...
        try:
            ...
            raise DuplicateResourceError(CUSTOMER_PHONE_ALREADY_EXISTS) from error
    ...
    async def update(
        ...
        phone_owner = await self._customers.get_by_business_and_phone(business_id, payload.phone)
    ...
    async def portfolio(self, business_id: UUID) -> builtins.list[CustomerPortfolioEntry]:
        ...
        by_customer: dict[UUID, builtins.list[Appointment]] = {}
        for appointment in appointments:
            by_customer.setdefault(appointment.customer_id, []).append(appointment)

        rows: builtins.list[CustomerPortfolioEntry] = []
        for customer in customers:
            history = by_customer.get(customer.id, [])
            completed = [item for item in history if item.status == AppointmentStatus.COMPLETED]
            rows.append(
                CustomerPortfolioEntry(
                    ...
                    no_show=sum(item.status == AppointmentStatus.NO_SHOW for item in history),
                    total_spent_cents=sum(item.price_cents or 0 for item in completed),
                    last_visit_at=max((item.starts_at for item in completed), default=None),
    ...
    async def redeem_loyalty_reward(self, business_id: UUID, customer_id: UUID) -> None:
        ...
        completed = sum(item.status == AppointmentStatus.COMPLETED for item in history)
        earned = completed // business.loyalty_target
        if earned <= customer.loyalty_rewards_redeemed:
            raise LoyaltyRewardUnavailableError("This customer has no loyalty reward available.")
